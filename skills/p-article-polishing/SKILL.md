...
表达库和章节功能骨架在同级的 `p-article-shared` 包里，与 `p-article-evaluated` 共用一份。该包不存在时按下面四条自检照常执行，并提示用户安装 `p-article-shared`。
...
- 改已有文本的语言、比较几种表达 → 按下面四条自检改；遇到拿不准的选择（动词强度、限定词、空白类型、因果强度、连接词）再查 [../p-article-shared/expression.md](../p-article-shared/expression.md)。**先写句子后查表**，反过来就会为了套句式而扭曲结论。
- 写图注或题目 → 功能顺序见 [../p-article-shared/section-skeleton.md](../p-article-shared/section-skeleton.md)。
...
- 新增参考论文或新的修改记录 → 读 [../p-article-shared/corpus.md](../p-article-shared/corpus.md)。
...
- **反向同样成立：不掺入模型腔。**不为显得学术而引入目标稿没有的模型化用词；不用破折号（`—`、`–`）插入解释，冒号尽量不用于句中引出解释，标点优先逗号和句号、分号少用。破折号不是连字符——`FCC- and HCP-`、`low-SFE` 这类构词属于上一条的作者约定，保留不动。完整词表和标点细则见 [../p-article-shared/expression.md](../p-article-shared/expression.md)。
...
**其余直接改**，事后一行说明。四条自检或 [../p-article-shared/expression.md](../p-article-shared/expression.md) 已能判定的（证据强度决定的动词、缺落点、体例一致性）不算阻断问题——没有阻断问题时不要凑，直接交付润色结果。把有默认答案的问题抛给作者，是把工作退回去。
...
- 每条记：日期、原句、给过的选项、作者的选择和理由、建议合入 `expression.md` 的哪张表。
- 只追加，不改动已有条目，交付时报告文件绝对路径。
- 没有稿件文件时（例如作者直接粘贴文本），不建文件，把这些条目写在交付说明里。

不要直接改 `../p-article-shared/expression.md`。安装后的 skill 是 `~/.agents/skills/` 下的副本，改在那里进不了 Git 仓库，由作者自行合并 `inbox.md`。
